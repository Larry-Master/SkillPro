...
describe('Submission model', () => {
  ...
  it('should throw an error if student is missing', async () => {
    ...
    const validationError = new Error('Submission validation failed: student: Path `student` is required.');
    ...
    const submission = new Submission(mockSubmissionData);
    await expect(submission.save()).rejects.toThrow('Submission validation failed: student: Path `student` is required.');
    expect(mockSave).toHaveBeenCalled();
  });
  ...
  it('should throw an error if assignment is missing', async () => {
    ...
    const validationError = new Error('Submission validation failed: assignment: Path `assignment` is required.');
    validationError.name = 'ValidationError';
    ...
    const submission = new Submission(mockSubmissionData);
    await expect(submission.save()).rejects.toThrow('Submission validation failed: assignment: Path `assignment` is required.');
    expect(mockSave).toHaveBeenCalled();
  });
  ...
  it('should set submittedAt to default if not provided', async () => {
    ...
    const mockSave = jest.fn().mockResolvedValue({ ...mockSubmissionData, submittedAt: mockNow });
    Submission.mockImplementation(() => ({
      save: mockSave
    }));
